...
import {BegonaUtlarTuri} from '../../classes/interfaceSpr';
...
@Component({
  selector: 'app-begona-utlar-turi',
  templateUrl: './begona-utlar-turi.component.html',
  styleUrls: ['../../../assets/styles/spravochniks.css']
})
export class BegonaUtlarTuriComponent implements OnInit {
  displayedColumns: string[] = ['id', 'nomi'];
  filterBegonaUtlarTuri: FilterName = {};
  ...
  isVisbleForm = false;
  selectedItem: BegonaUtlarTuri;
  reloading = false;
  positionId: string;
  className = 'begonaUtlarTuri';
  list: BegonaUtlarTuri;
  page = 1;
  pageSize = 10;
  totalPages: number;
  totalDocs: number;
  ...
  opeFilterDialog(): void {
    const dialogRef = this.dialogFilter.open(FilterNameSprComponent, {
      width: '350px',
      data: {name: this.filterBegonaUtlarTuri.name, SPR: 'Бегона ўтлар тури номи'}
    });
    dialogRef.afterClosed().subscribe(result => {
      ...
      if (result.name && result.ok) {
        this.filterBegonaUtlarTuri.name = result.name;
        this.getByFilter();
      } else {
        if (!result.close) {
          this.filterBegonaUtlarTuri.name = null;
          this.getAll();
        }
      }
    });
  ...
  onSubmit() {
    const newBegonaUtlarTuri: BegonaUtlarTuri = {
      name: this.form.value.name,
      tartibRaqam: this.form.value.tartibRaqam,
    };
    if (newBegonaUtlarTuri) {
      this.creatAll(newBegonaUtlarTuri);
  ...
  creatAll(ObjectForm) {
    if (this.positionId) {
      ...
      this.apiService.updated<BegonaUtlarTuri>(this.className, ObjectForm, ObjectForm._id)
        .subscribe(
      ...
      this.apiService.creted<BegonaUtlarTuri>(this.className, ObjectForm)
        .subscribe(
  ...
  getAll() {
    this.reloading = true;
    const params = Object.assign({}, this.filterBegonaUtlarTuri, {
      page: this.page,
      pageSize: this.pageSize
    });
    this.apiService.getAllByPagePost<PageResponse<BegonaUtlarTuri>>(params, this.className)
      .subscribe(result => {
  ...
  getByFilter() {
    ...
    this.apiService.getFilter<PageResponse<BegonaUtlarTuri>>(params, this.className)
      .subscribe(result => {
        this.list = result.content;
        this.totalDocs = result.totalDocs;
        this.hasPrevPage = result.hasPrevPage;
