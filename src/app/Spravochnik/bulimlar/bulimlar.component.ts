...
import {Bulimlar} from '../../classes/interfaceSpr';
...
@Component({
  selector: 'app-bulimlar',
  templateUrl: './bulimlar.component.html',
  styleUrls: ['../../../assets/styles/spravochniks.css']
})
export class BulimlarComponent implements OnInit {
  displayedColumns: string[] = ['id', 'nomi'];
  filterBulimlar: FilterName = {};

  Buttonlar: boolean;
  form: FormGroup;
  tartibRaqamLength: number;
  isVisbleForm = false;
  selectedItem: Bulimlar;
  reloading = false;
  positionId: string;
  className = 'bulimlar';
  list: Bulimlar;
  ...
  opeFilterDialog(): void {
    const dialogRef = this.dialogFilter.open(FilterNameSprComponent, {
      width: '350px',
      data: {name: this.filterBulimlar.name, SPR: 'Бўлим номи'}
    });
    dialogRef.afterClosed().subscribe(result => {
      ...
      if (result.name && result.ok) {
        this.filterBulimlar.name = result.name;
        this.getByFilter();
      } else {
        if (!result.close) {
          this.filterBulimlar.name = null;
          this.getAll();
          this.EnableButton();
        }
      }
  ...
  onSubmit() {
    const newBulimlar: Bulimlar = {
      name: this.form.value.name,
      tartibRaqam: this.form.value.tartibRaqam,
    };
    if (newBulimlar) {
      this.creatAll(newBulimlar);
  ...
  creatAll(ObjectForm) {
    if (this.positionId) {
      ...
      this.apiService.updated<Bulimlar>(this.className, ObjectForm, ObjectForm._id)
        .subscribe(
      ...
      this.apiService.creted<Bulimlar>(this.className, ObjectForm)
        .subscribe(
  ...
  getAll() {
    this.reloading = true;
    const params = Object.assign({}, this.filterBulimlar, {
      page: this.page,
      pageSize: this.pageSize
    });
    this.apiService.getAllByPagePost<PageResponse<Bulimlar>>(params, this.className)
      .subscribe(result => {
  ...
  getByFilter() {
    ...
    this.apiService.getFilter<PageResponse<Bulimlar>>(params, this.className)
      .subscribe(result => {
        this.list = result.content;
        this.totalDocs = result.totalDocs;
        this.hasPrevPage = result.hasPrevPage;
