...
import {Bulinmalar} from '../../classes/interfaceSpr';
...
@Component({
  selector: 'app-bulinmalar',
  templateUrl: './bulinmalar.component.html',
  styleUrls: ['../../../assets/styles/spravochniks.css']
})
export class BulinmalarComponent implements OnInit {
  displayedColumns: string[] = ['id', 'nomi'];
  filterBulinmalar: FilterName = {};

  Buttonlar: boolean;
  form: FormGroup;
  tartibRaqamLength: number;
  isVisbleForm = false;
  selectedItem: Bulinmalar;
  reloading = false;
  positionId: string;
  className = 'bulinmalar';
  list: Bulinmalar;
  page = 1;
  pageSize = 10;
  totalPages: number;
  ...
  opeFilterDialog(): void {
    const dialogRef = this.dialogFilter.open(FilterNameSprComponent, {
      width: '350px',
      data: {name: this.filterBulinmalar.name, SPR: 'Бўлинма номи'}
    });
    dialogRef.afterClosed().subscribe(result => {
      ...
      if (result.name && result.ok) {
        this.filterBulinmalar.name = result.name;
        this.getByFilter();
      } else {
        if (!result.close) {
          this.filterBulinmalar.name = null;
          this.getAll();
          this.EnableButton();
        }
      }
  ...
  onSubmit() {
    const newBulinmalar: Bulinmalar = {
      name: this.form.value.name,
      tartibRaqam: this.form.value.tartibRaqam,
    };
    if (newBulinmalar) {
      this.creatAll(newBulinmalar);
  ...
  creatAll(ObjectForm) {
    if (this.positionId) {
      ...
      this.apiService.updated<Bulinmalar>(this.className, ObjectForm, ObjectForm._id)
        .subscribe(
      ...
      this.apiService.creted<Bulinmalar>(this.className, ObjectForm)
        .subscribe(
  ...
  getAll() {
    this.reloading = true;
    const params = Object.assign({}, this.filterBulinmalar, {
      page: this.page,
      pageSize: this.pageSize
    });
    this.apiService.getAllByPagePost<PageResponse<Bulinmalar>>(params, this.className)
      .subscribe(result => {
  ...
  getByFilter() {
    ...
    this.apiService.getFilter<PageResponse<Bulinmalar>>(params, this.className)
      .subscribe(result => {
        this.list = result.content;
        this.totalDocs = result.totalDocs;
        this.hasPrevPage = result.hasPrevPage;
