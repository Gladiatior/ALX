...
import {Davlatlar} from '../../classes/interfaceSpr';
...
@Component({
  selector: 'app-davlatlar',
  templateUrl: './davlatlar.component.html',
  styleUrls: ['../../../assets/styles/spravochniks.css']
})
export class DavlatlarComponent implements OnInit {
  displayedColumns: string[] = ['id', 'nomi', 'qisqacha'];
  filterDavlatlar: FilterName = {};

  Buttonlar: boolean;
  form: FormGroup;
  tartibRaqamLength: number;
  isVisbleForm = false;
  selectedItem: Davlatlar;
  reloading = false;
  positionId: string;
  className = 'davlatlar';
  list: Davlatlar;
  ...
  ngOnInit() {
    this.form = new FormGroup({
      ...
      qisqachaNomi: new FormControl(null, Validators.required),
    });
    this.getAll();
  }
  ...
  opeFilterDialog(): void {
    const dialogRef = this.dialogFilter.open(FilterNameSprComponent, {
      width: '350px',
      data: {name: this.filterDavlatlar.name, SPR: 'Давлат номи'}
    });
    dialogRef.afterClosed().subscribe(result => {
      ...
      if (result.name && result.ok) {
        this.filterDavlatlar.name = result.name;
        this.getByFilter();
      } else {
        if (!result.close) {
          this.filterDavlatlar.name = null;
          this.getAll();
          this.EnableButton();
        }
      }
  ...
  onSubmit() {
    const newDavlatlar: Davlatlar = {
      name: this.form.value.name,
      tartibRaqam: this.form.value.tartibRaqam,
      qisqachaNomi: this.form.value.qisqachaNomi
    };
    if (newDavlatlar) {
      this.creatAll(newDavlatlar);
  ...
  editObject() {
    if (this.selectedItem) {
      this.form.patchValue({
        ...
        qisqachaNomi: this.selectedItem.qisqachaNomi
      });
    } else {
  ...
  creatAll(ObjectForm) {
    if (this.positionId) {
      ...
      this.apiService.updated<Davlatlar>(this.className, ObjectForm, ObjectForm._id)
        .subscribe(
      ...
      this.apiService.creted<Davlatlar>(this.className, ObjectForm)
        .subscribe(
  ...
  getAll() {
    this.reloading = true;
    const params = Object.assign({}, this.filterDavlatlar, {
      page: this.page,
      pageSize: this.pageSize
    });
    this.apiService.getAllByPagePost<PageResponse<Davlatlar>>(params, this.className)
      .subscribe(result => {
  ...
  getByFilter() {
    ...
    this.apiService.getFilter<PageResponse<Davlatlar>>(params, this.className)
      .subscribe(result => {
        this.list = result.content;
        this.totalDocs = result.totalDocs;
        this.hasPrevPage = result.hasPrevPage;
