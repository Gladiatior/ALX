...
import {FaoliyatTuri} from '../../classes/interfaceSpr';
...
@Component({
  selector: 'app-faoliyat-turi',
  templateUrl: './faoliyat-turi.component.html',
  styleUrls: ['../../../assets/styles/spravochniks.css']
})
export class FaoliyatTuriComponent implements OnInit {
  displayedColumns: string[] = ['id', 'nomi'];
  filterFaoliyatTuri: FilterName = {};

  Buttonlar: boolean;
  form: FormGroup;
  tartibRaqamLength: number;
  isVisbleForm = false;
  selectedItem: FaoliyatTuri;
  reloading = false;
  positionId: string;
  className = 'faoliyatTuri';
  list: FaoliyatTuri;
  page = 1;
  pageSize = 10;
  totalPages: number;
  ...
  opeFilterDialog(): void {
    const dialogRef = this.dialogFilter.open(FilterNameSprComponent, {
      width: '350px',
      data: {name: this.filterFaoliyatTuri.name, SPR: 'Фаолият тури номи'}
    });
    dialogRef.afterClosed().subscribe(result => {
      ...
      if (result.name && result.ok) {
        this.filterFaoliyatTuri.name = result.name;
        this.getByFilter();
      } else {
        if (!result.close) {
          this.filterFaoliyatTuri.name = null;
          this.getAll();
        }
      }
    });
  ...
  onSubmit() {
    const newFaoliyatTuri: FaoliyatTuri = {
      name: this.form.value.name,
      tartibRaqam: this.form.value.tartibRaqam,
    };
    if (newFaoliyatTuri) {
      this.creatAll(newFaoliyatTuri);
  ...
  creatAll(ObjectForm) {
    if (this.positionId) {
      ...
      this.apiService.updated<FaoliyatTuri>(this.className, ObjectForm, ObjectForm._id)
        .subscribe(
      ...
      this.apiService.creted<FaoliyatTuri>(this.className, ObjectForm)
        .subscribe(
  ...
  getAll() {
    this.reloading = true;
    const params = Object.assign({}, this.filterFaoliyatTuri, {
      page: this.page,
      pageSize: this.pageSize
    });
    this.apiService.getAllByPagePost<PageResponse<FaoliyatTuri>>(params, this.className)
      .subscribe(result => {
  ...
  getByFilter() {
    ...
    this.apiService.getFilter<PageResponse<FaoliyatTuri>>(params, this.className)
      .subscribe(result => {
        this.list = result.content;
        this.totalDocs = result.totalDocs;
        this.hasPrevPage = result.hasPrevPage;
