...
import {FumigatsiyaForSpravochnik, FumigatsiyaTuri, UlchovBirligi} from '../../classes/interfaceSpr';
...
@Component({
  selector: 'app-fumigatsiya-for-spravochnik',
  templateUrl: './fumigatsiya-for-spravochnik.component.html',
  styleUrls: ['../../../assets/styles/spravochniks.css']
})
export class FumigatsiyaForSpravochnikComponent implements OnInit {
  displayedColumns: string[] = ['id', 'nomi', 'ulchovBirligi', 'narxi', 'fumigatsiyaTuri'];
  ...
  filterFumigatsiyaForSpravochnik: FilterName = {};

  Buttonlar: boolean;
  form: FormGroup;
  tartibRaqamLength: number;
  isVisbleForm = false;
  selectedItem: FumigatsiyaForSpravochnik;
  reloading = false;
  positionId: string;
  className = 'fumigatsiyaForSpravochnik';
  list: FumigatsiyaForSpravochnik;
  page = 1;
  pageSize = 10;
  totalPages: number;
  totalDocs: number;
  ...
  opeFilterDialog(): void {
    const dialogRef = this.dialogFilter.open(FilterNameSprComponent, {
      width: '350px',
      data: {name: this.filterFumigatsiyaForSpravochnik.name, SPR: 'Фумигатсия номи'}
    });
    dialogRef.afterClosed().subscribe(result => {
      ...
      if (result.name && result.ok) {
        this.filterFumigatsiyaForSpravochnik.name = result.name;
        this.getByFilter();
      } else {
        if (!result.close) {
          this.filterFumigatsiyaForSpravochnik.name = null;
          this.getAll();
          this.EnableButton();
        }
      }
  ...
  onSubmit() {
    const newFumigatsiyaForSpravochnik: FumigatsiyaForSpravochnik = {
    ...
    if (newFumigatsiyaForSpravochnik) {
      this.creatAll(newFumigatsiyaForSpravochnik);
  ...
  onResetForm() {
    this.filterFumigatsiyaForSpravochnik = {};
    this.selectedItem = null;
    this.isVisbleForm = false;
    this.getFumitagsiyaTuri();
    this.getUlchovBirligi();
  ...
  creatAll(ObjectForm) {
    if (this.positionId) {
      ...
      this.apiService.updated<FumigatsiyaForSpravochnik>(this.className, ObjectForm, ObjectForm._id)
        .subscribe(
      ...
      this.apiService.creted<FumigatsiyaForSpravochnik>(this.className, ObjectForm)
        .subscribe(
  ...
  getAll() {
    this.reloading = true;
    const params = Object.assign({}, this.filterFumigatsiyaForSpravochnik, {
      page: this.page,
      pageSize: this.pageSize
    });
    this.apiService.getAllByPagePost<PageResponse<FumigatsiyaForSpravochnik>>(params, this.className)
      .subscribe(result => {
  ...
  getByFilter() {
    ...
    this.apiService.getFilter<PageResponse<FumigatsiyaForSpravochnik>>(params, this.className)
      .subscribe(result => {
        this.list = result.content;
        this.totalDocs = result.totalDocs;
        this.hasPrevPage = result.hasPrevPage;
