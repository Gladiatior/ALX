...
import {FumigatsiyaTuri} from '../../classes/interfaceSpr';
...
@Component({
  selector: 'app-fumigatsiya-turi',
  templateUrl: './fumigatsiya-turi.component.html',
  styleUrls: ['../../../assets/styles/spravochniks.css']
})
export class FumigatsiyaTuriComponent implements OnInit {
  displayedColumns: string[] = ['id', 'nomi', 'formula'];
  filterFumigatsiyaTuri: FilterName = {};

  Buttonlar: boolean;
  form: FormGroup;
  tartibRaqamLength: number;
  isVisbleForm = false;
  selectedItem: FumigatsiyaTuri;
  reloading = false;
  positionId: string;
  className = 'fumigatsiyaTuri';
  list: FumigatsiyaTuri;
  page = 1;
  pageSize = 10;
  totalPages: number;
  totalDocs: number;
  ...
  opeFilterDialog(): void {
    const dialogRef = this.dialogFilter.open(FilterNameSprComponent, {
      width: '350px',
      data: {name: this.filterFumigatsiyaTuri.name, SPR: 'Фумигатсия тури номи'}
    });
    dialogRef.afterClosed().subscribe(result => {
      ...
      if (result.name && result.ok) {
        this.filterFumigatsiyaTuri.name = result.name;
        this.getByFilter();
      } else {
        if (!result.close) {
          this.filterFumigatsiyaTuri.name = null;
          this.getAll();
          this.EnableButton();
        }
      }
  ...
  onSubmit() {
    const newFumigatsiyaTuri: FumigatsiyaTuri = {
      name: this.form.value.name,
      tartibRaqam: this.form.value.tartibRaqam,
      formula: this.form.value.formula
    };
    if (newFumigatsiyaTuri) {
      this.creatAll(newFumigatsiyaTuri);
  ...
  creatAll(ObjectForm) {
    if (this.positionId) {
      ...
      this.apiService.updated<FumigatsiyaTuri>(this.className, ObjectForm, ObjectForm._id)
        .subscribe(
      ...
      this.apiService.creted<FumigatsiyaTuri>(this.className, ObjectForm)
        .subscribe(
  ...
  getAll() {
    this.reloading = true;
    const params = Object.assign({}, this.filterFumigatsiyaTuri, {
      page: this.page,
      pageSize: this.pageSize
    });
    this.apiService.getAllByPagePost<PageResponse<FumigatsiyaTuri>>(params, this.className)
      .subscribe(result => {
  ...
  getByFilter() {
    ...
    this.apiService.getFilter<PageResponse<FumigatsiyaTuri>>(params, this.className)
      .subscribe(result => {
        this.list = result.content;
        this.totalDocs = result.totalDocs;
        this.hasPrevPage = result.hasPrevPage;
