...
import {KasalliklarTuri} from '../../classes/interfaceSpr';
...
@Component({
  selector: 'app-kasalliklar-turi',
  templateUrl: './kasalliklar-turi.component.html',
  styleUrls: ['../../../assets/styles/spravochniks.css']
})
export class KasalliklarTuriComponent implements OnInit {
  displayedColumns: string[] = ['id', 'nomi'];
  filterKasalliklarTuri: FilterName = {};

  Buttonlar: boolean;
  form: FormGroup;
  tartibRaqamLength: number;
  isVisbleForm = false;
  selectedItem: KasalliklarTuri;
  reloading = false;
  positionId: string;
  className = 'kasalliklarTuri';
  list: KasalliklarTuri;
  page = 1;
  pageSize = 10;
  totalPages: number;
  totalDocs: number;
  ...
  opeFilterDialog(): void {
    const dialogRef = this.dialogFilter.open(FilterNameSprComponent, {
      width: '350px',
      data: {name: this.filterKasalliklarTuri.name, SPR: 'Касалликлар тури номи'}
    });
    dialogRef.afterClosed().subscribe(result => {
      ...
      if (result.name && result.ok) {
        this.filterKasalliklarTuri.name = result.name;
        this.getByFilter();
      } else {
        if (!result.close) {
          this.filterKasalliklarTuri.name = null;
          this.getAll();
          this.EnableButton();
        }
      }
  ...
  onSubmit() {
    const newKasalliklarTuri: KasalliklarTuri = {
      name: this.form.value.name,
      tartibRaqam: this.form.value.tartibRaqam,
    };
    if (newKasalliklarTuri) {
      this.creatAll(newKasalliklarTuri);
  ...
  creatAll(ObjectForm) {
    if (this.positionId) {
      ...
      this.apiService.updated<KasalliklarTuri>(this.className, ObjectForm, ObjectForm._id)
        .subscribe(
      ...
      this.apiService.creted<KasalliklarTuri>(this.className, ObjectForm)
        .subscribe(
  ...
  getAll() {
    this.reloading = true;
    const params = Object.assign({}, this.filterKasalliklarTuri, {
      page: this.page,
      pageSize: this.pageSize
    });
    this.apiService.getAllByPagePost<PageResponse<KasalliklarTuri>>(params, this.className)
      .subscribe(result => {
  ...
  getByFilter() {
    ...
    this.apiService.getFilter<PageResponse<KasalliklarTuri>>(params, this.className)
      .subscribe(result => {
        this.list = result.content;
        this.totalDocs = result.totalDocs;
        this.hasPrevPage = result.hasPrevPage;
