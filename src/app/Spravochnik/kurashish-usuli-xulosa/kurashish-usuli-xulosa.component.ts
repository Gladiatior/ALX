...
import {KurashishUsuliXulosa} from '../../classes/interfaceSpr';
...
@Component({
  selector: 'app-kurashish-usuli-xulosa',
  templateUrl: './kurashish-usuli-xulosa.component.html',
  styleUrls: ['../../../assets/styles/spravochniks.css']
})
export class KurashishUsuliXulosaComponent implements OnInit {
  displayedColumns: string[] = ['id', 'nomi'];
  filterKurashishXulosa: FilterName = {};

  Buttonlar: boolean;
  form: FormGroup;
  tartibRaqamLength: number;
  isVisbleForm = false;
  selectedItem: KurashishUsuliXulosa;
  reloading = false;
  positionId: string;
  className = 'kurashishUsuliXulosa';
  list: KurashishUsuliXulosa;
  page = 1;
  pageSize = 10;
  totalPages: number;
  totalDocs: number;
  ...
  opeFilterDialog(): void {
    const dialogRef = this.dialogFilter.open(FilterNameSprComponent, {
      width: '350px',
      data: {name: this.filterKurashishXulosa.name, SPR: 'Курашиши усули хулосаси номи'}
    });
    dialogRef.afterClosed().subscribe(result => {
      ...
      if (result.name && result.ok) {
        this.filterKurashishXulosa.name = result.name;
        this.getByFilter();
      } else {
        if (!result.close) {
          this.filterKurashishXulosa.name = null;
          this.getAll();
        }
      }
    });
  ...
  onSubmit() {
    const newKurashishXulosasi: KurashishUsuliXulosa = {
      name: this.form.value.name,
      tartibRaqam: this.form.value.tartibRaqam,
    };
    if (newKurashishXulosasi) {
      this.creatAll(newKurashishXulosasi);
  ...
  creatAll(ObjectForm) {
    if (this.positionId) {
      ...
      this.apiService.updated<KurashishUsuliXulosa>(this.className, ObjectForm, ObjectForm._id)
        .subscribe(
      ...
      this.apiService.creted<KurashishUsuliXulosa>(this.className, ObjectForm)
        .subscribe(
  ...
  getAll() {
    this.reloading = true;
    const params = Object.assign({}, this.filterKurashishXulosa, {
      page: this.page,
      pageSize: this.pageSize
    });
    this.apiService.getAllByPagePost<PageResponse<KurashishUsuliXulosa>>(params, this.className)
      .subscribe(result => {
  ...
  getByFilter() {
    ...
    this.apiService.getFilter<PageResponse<KurashishUsuliXulosa>>(params, this.className)
      .subscribe(result => {
        this.list = result.content;
        this.totalDocs = result.totalDocs;
        this.hasPrevPage = result.hasPrevPage;
