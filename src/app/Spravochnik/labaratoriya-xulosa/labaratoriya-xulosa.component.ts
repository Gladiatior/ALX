...
import {LabaratoriyaXulosa} from '../../classes/interfaceSpr';
...
@Component({
  selector: 'app-labaratoriya-xulosa',
  templateUrl: './labaratoriya-xulosa.component.html',
  styleUrls: ['../../../assets/styles/spravochniks.css']
})
export class LabaratoriyaXulosaComponent implements OnInit {
  displayedColumns: string[] = ['id', 'nomi'];
  filterLabaratoriyaXulosa: FilterName = {};

  Buttonlar: boolean;
  form: FormGroup;
  tartibRaqamLength: number;
  isVisbleForm = false;
  selectedItem: LabaratoriyaXulosa;
  reloading = false;
  positionId: string;
  className = 'labaratoriyaXulosa';
  list: LabaratoriyaXulosa;
  page = 1;
  pageSize = 10;
  totalPages: number;
  totalDocs: number;
  ...
  opeFilterDialog(): void {
    const dialogRef = this.dialogFilter.open(FilterNameSprComponent, {
      width: '350px',
      data: {name: this.filterLabaratoriyaXulosa.name, SPR: 'Лабаратория хулоса номи'}
    });
    dialogRef.afterClosed().subscribe(result => {
      ...
      if (result.name && result.ok) {
        this.filterLabaratoriyaXulosa.name = result.name;
        this.getByFilter();
      } else {
        if (!result.close) {
          this.filterLabaratoriyaXulosa.name = null;
          this.getAll();
        }
      }
    });
  ...
  onSubmit() {
    const newLabaratoriyaXulosa: LabaratoriyaXulosa = {
      name: this.form.value.name,
      tartibRaqam: this.form.value.tartibRaqam,
    };
    if (newLabaratoriyaXulosa) {
      this.creatAll(newLabaratoriyaXulosa);
  ...
  creatAll(ObjectForm) {
    if (this.positionId) {
      ...
      this.apiService.updated<LabaratoriyaXulosa>(this.className, ObjectForm, ObjectForm._id)
        .subscribe(
      ...
      this.apiService.creted<LabaratoriyaXulosa>(this.className, ObjectForm)
        .subscribe(
  ...
  getAll() {
    this.reloading = true;
    const params = Object.assign({}, this.filterLabaratoriyaXulosa, {
      page: this.page,
      pageSize: this.pageSize
    });
    this.apiService.getAllByPagePost<PageResponse<LabaratoriyaXulosa>>(params, this.className)
      .subscribe(result => {
  ...
  getByFilter() {
    ...
    this.apiService.getFilter<PageResponse<LabaratoriyaXulosa>>(params, this.className)
      .subscribe(result => {
        this.list = result.content;
        this.totalDocs = result.totalDocs;
        this.hasPrevPage = result.hasPrevPage;
