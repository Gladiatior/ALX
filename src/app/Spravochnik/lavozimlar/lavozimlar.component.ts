...
import {Lavozimlar} from '../../classes/interfaceSpr';
...
@Component({
  selector: 'app-lavozimlar',
  templateUrl: './lavozimlar.component.html',
  styleUrls: ['../../../assets/styles/spravochniks.css']
})
export class LavozimlarComponent implements OnInit {
  displayedColumns: string[] = ['id', 'nomi'];
  filterLavozimlar: FilterName = {};

  Buttonlar: boolean;
  form: FormGroup;
  tartibRaqamLength: number;
  isVisbleForm = false;
  selectedItem: Lavozimlar;
  reloading = false;
  positionId: string;
  className = 'lavozimlar';
  list: Lavozimlar;
  page = 1;
  pageSize = 10;
  totalPages: number;
  ...
  opeFilterDialog(): void {
    const dialogRef = this.dialogFilter.open(FilterNameSprComponent, {
      width: '350px',
      data: {name: this.filterLavozimlar.name, SPR: 'Лавозим номи'}
    });
    dialogRef.afterClosed().subscribe(result => {
      ...
      if (result.name && result.ok) {
        this.filterLavozimlar.name = result.name;
        this.getByFilter();
      } else {
        if (!result.close) {
          this.filterLavozimlar.name = null;
          this.getAll();
          this.EnableButton();
        }
      }
  ...
  onSubmit() {
    const newLavozimlar: Lavozimlar = {
      name: this.form.value.name,
      tartibRaqam: this.form.value.tartibRaqam,
    };
    if (newLavozimlar) {
      this.creatAll(newLavozimlar);
  ...
  creatAll(ObjectForm) {
    if (this.positionId) {
      ...
      this.apiService.updated<Lavozimlar>(this.className, ObjectForm, ObjectForm._id)
        .subscribe(
      ...
      this.apiService.creted<Lavozimlar>(this.className, ObjectForm)
        .subscribe(
  ...
  getAll() {
    this.reloading = true;
    const params = Object.assign({}, this.filterLavozimlar, {
      page: this.page,
      pageSize: this.pageSize
    });
    this.apiService.getAllByPagePost<PageResponse<Lavozimlar>>(params, this.className)
      .subscribe(result => {
  ...
  getByFilter() {
    ...
    this.apiService.getFilter<PageResponse<Lavozimlar>>(params, this.className)
      .subscribe(result => {
        this.list = result.content;
        this.totalDocs = result.totalDocs;
        this.hasPrevPage = result.hasPrevPage;
