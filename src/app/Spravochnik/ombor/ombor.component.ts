...
import {Omborlar} from '../../classes/interfaceSpr';
...
export class OmborComponent implements OnInit {
  displayedColumns: string[] = ['id', 'nomi', 'javobgarShaxs'];
  filterOmborlar: FilterName = {};

  Buttonlar: boolean;
  form: FormGroup;
  tartibRaqamLength: number;
  isVisbleForm = false;
  selectedItem: Omborlar;
  reloading = false;
  positionId: string;
  className = 'omborlar';
  list: Omborlar;
  ...
  ngOnInit() {
    this.form = new FormGroup({
      ...
      javobgarShaxs: new FormControl(null, Validators.required)
    });
    this.getAll();
  }
  ...
  opeFilterDialog(): void {
    const dialogRef = this.dialogFilter.open(FilterNameSprComponent, {
      width: '350px',
      data: {name: this.filterOmborlar.name, SPR: 'Омбор номи'}
    });
    dialogRef.afterClosed().subscribe(result => {
      ...
      if (result.name && result.ok) {
        this.filterOmborlar.name = result.name;
        this.getByFilter();
      } else {
        if (!result.close) {
          this.filterOmborlar.name = null;
          this.getAll();
          this.EnableButton();
        }
      }
  ...
  onSubmit() {
    const newOmborlar: Omborlar = {
      name: this.form.value.name,
      tartibRaqam: this.form.value.tartibRaqam,
      javobgarShaxs: this.form.value.javobgarShaxs
    };
    if (newOmborlar) {
      this.creatAll(newOmborlar);
  ...
  editObject() {
    if (this.selectedItem) {
      this.form.patchValue({
        ...
        javobgarShaxs: this.selectedItem.javobgarShaxs
      });
    } else {
  ...
  creatAll(ObjectForm) {
    if (this.positionId) {
      ...
      this.apiService.updated<Omborlar>(this.className, ObjectForm, ObjectForm._id)
        .subscribe(
      ...
      this.apiService.creted<Omborlar>(this.className, ObjectForm)
        .subscribe(
  ...
  getAll() {
    this.reloading = true;
    const params = Object.assign({}, this.filterOmborlar, {
      page: this.page,
      pageSize: this.pageSize
    });
    this.apiService.getAllByPagePost<PageResponse<Omborlar>>(params, this.className)
      .subscribe(result => {
  ...
  getByFilter() {
    ...
    this.apiService.getFilter<PageResponse<Omborlar>>(params, this.className)
      .subscribe(result => {
        this.list = result.content;
        this.totalDocs = result.totalDocs;
        this.hasPrevPage = result.hasPrevPage;
