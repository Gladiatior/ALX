...
import {Qadoqlar} from '../../classes/interfaceSpr';
...
@Component({
  selector: 'app-qadoqlar',
  templateUrl: './qadoqlar.component.html',
  styleUrls: ['../../../assets/styles/spravochniks.css']
})
export class QadoqlarComponent implements OnInit {
  displayedColumns: string[] = ['id', 'nomi'];
  filterQadoqlar: FilterName = {};

  Buttonlar: boolean;
  form: FormGroup;
  tartibRaqamLength: number;
  isVisbleForm = false;
  selectedItem: Qadoqlar;
  reloading = false;
  positionId: string;
  className = 'qadoqlar';
  list: Qadoqlar;
  ...
  opeFilterDialog(): void {
    const dialogRef = this.dialogFilter.open(FilterNameSprComponent, {
      width: '350px',
      data: {name: this.filterQadoqlar.name, SPR: 'Бўлим номи'}
    });
    dialogRef.afterClosed().subscribe(result => {
      ...
      if (result.name && result.ok) {
        this.filterQadoqlar.name = result.name;
        this.getByFilter();
      } else {
        if (!result.close) {
          this.filterQadoqlar.name = null;
          this.getAll();
          this.EnableButton();
        }
      }
  ...
  onSubmit() {
    const newQadoqlar: Qadoqlar = {
      name: this.form.value.name,
      tartibRaqam: this.form.value.tartibRaqam,
    };
    if (newQadoqlar) {
      this.creatAll(newQadoqlar);
  ...
  creatAll(ObjectForm) {
    if (this.positionId) {
      ...
      this.apiService.updated<Qadoqlar>(this.className, ObjectForm, ObjectForm._id)
        .subscribe(
      ...
      this.apiService.creted<Qadoqlar>(this.className, ObjectForm)
        .subscribe(
  ...
  getAll() {
    this.reloading = true;
    const params = Object.assign({}, this.filterQadoqlar, {
      page: this.page,
      pageSize: this.pageSize
    });
    this.apiService.getAllByPagePost<PageResponse<Qadoqlar>>(params, this.className)
      .subscribe(result => {
  ...
  getByFilter() {
    ...
    this.apiService.getFilter<PageResponse<Qadoqlar>>(params, this.className)
      .subscribe(result => {
        this.list = result.content;
        this.totalDocs = result.totalDocs;
        this.hasPrevPage = result.hasPrevPage;
