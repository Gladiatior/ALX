...
import {Qfylar, Tumanlar, Viloyatlar} from '../../classes/interfaceSpr';
...
@Component({
  selector: 'app-qfy',
  templateUrl: './qfy.component.html',
  styleUrls: ['../../../assets/styles/spravochniks.css']
})
export class QFYComponent implements OnInit {
  ...
  opeFilterDialog(): void {
    ...
    const dialogRef = this.dialogFilter.open(FilterNameSprComponent, {
      width: '350px',
      data: {name: this.filterQfylar.name, SPR: 'ҚФЙ номи'}
    });
    dialogRef.afterClosed().subscribe(result => {
      ...
      if (result.name && result.ok) {
        this.filterQfylar.name = result.name;
        this.getByFilter();
      } else {
        if (!result.close) {
          this.filterQfylar.name = null;
          this.getViloyatlar();
          this.getAll();
          this.EnableButton();
        }
  ...
  creatAll(ObjectForm) {
    if (this.positionId) {
      ...
      this.apiService.updated<Qfylar>(this.className, ObjectForm, ObjectForm._id)
        .subscribe(
      ...
      this.apiService.creted<Qfylar>(this.className, ObjectForm)
        .subscribe(
  ...
  getAll() {
    this.reloading = true;
    const params = Object.assign({}, this.filterQfylar, {
      page: this.page,
      pageSize: this.pageSize
    });
    this.apiService.getAllByPagePost<PageResponse<Qfylar>>(params, this.className)
      .subscribe(result => {
  ...
  getByFilter() {
    ...
    this.apiService.getFilter<PageResponse<Qfylar>>(params, this.className)
      .subscribe(result => {
        this.list = result.content;
        this.totalDocs = result.totalDocs;
        this.hasPrevPage = result.hasPrevPage;
