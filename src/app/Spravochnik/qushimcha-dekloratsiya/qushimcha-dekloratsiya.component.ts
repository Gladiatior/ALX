...
import {QushimchaDekloratsiya} from '../../classes/interfaceSpr';
import {PageResponse} from '../../classes/pageResponse';

@Component({
  selector: 'app-qushimcha-dekloratsiya',
  templateUrl: './qushimcha-dekloratsiya.component.html',
  styleUrls: ['../../../assets/styles/spravochniks.css']
})
export class QushimchaDekloratsiyaComponent implements OnInit {
  displayedColumns: string[] = ['id', 'nomi'];
  filterQushimcha: FilterName = {};

  Buttonlar: boolean;
  form: FormGroup;
  tartibRaqamLength: number;
  isVisbleForm = false;
  selectedItem: QushimchaDekloratsiya;
  reloading = false;
  positionId: string;
  className = 'qushimchaDekloratsiya';
  list: QushimchaDekloratsiya;
  page = 1;
  pageSize = 10;
  totalPages: number;
  totalDocs: number;
  ...
  opeFilterDialog(): void {
    const dialogRef = this.dialogFilter.open(FilterNameSprComponent, {
      width: '350px',
      data: {name: this.filterQushimcha.name, SPR: 'Кўшимча деклорация номи'}
    });
    dialogRef.afterClosed().subscribe(result => {
      ...
      if (result.name && result.ok) {
        this.filterQushimcha.name = result.name;
        this.getByFilter();
      } else {
        if (!result.close) {
          this.filterQushimcha.name = null;
          this.getAll();
          this.EnableButton();
        }
      }
  ...
  onSubmit() {
    const newQushimcha: QushimchaDekloratsiya = {
      name: this.form.value.name,
      tartibRaqam: this.form.value.tartibRaqam,
    };
    if (newQushimcha) {
      this.creatAll(newQushimcha);
  ...
  creatAll(ObjectForm) {
    if (this.positionId) {
      ...
      this.apiService.updated<QushimchaDekloratsiya>(this.className, ObjectForm, ObjectForm._id)
        .subscribe(
      ...
      this.apiService.creted<QushimchaDekloratsiya>(this.className, ObjectForm)
        .subscribe(
  ...
  getAll() {
    this.reloading = true;
    const params = Object.assign({}, this.filterQushimcha, {
      page: this.page,
      pageSize: this.pageSize
    });
    this.apiService.getAllByPagePost<PageResponse<QushimchaDekloratsiya>>(params, this.className)
      .subscribe(result => {
  ...
  getByFilter() {
    ...
    this.apiService.getFilter<PageResponse<QushimchaDekloratsiya>>(params, this.className)
      .subscribe(result => {
        this.list = result.content;
        this.totalDocs = result.totalDocs;
        this.hasPrevPage = result.hasPrevPage;
