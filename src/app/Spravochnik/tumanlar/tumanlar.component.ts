...
import {Tumanlar, Viloyatlar} from '../../classes/interfaceSpr';
...
@Component({
  selector: 'app-tumanlar',
  templateUrl: './tumanlar.component.html',
  styleUrls: ['../../../assets/styles/spravochniks.css']
})
export class TumanlarComponent implements OnInit {
  displayedColumns: string[] = ['id', 'nomi', 'viloyat'];
  viloyatlar: Viloyatlar[] = [];
  viloyatlarName: Viloyatlar;
  filterTumanlar: FilterName = {};

  Buttonlar: boolean;
  form: FormGroup;
  tartibRaqamLength: number;
  isVisbleForm = false;
  selectedItem: Tumanlar;
  reloading = false;
  positionId: string;
  className = 'tumanlar';
  list: Tumanlar;
  ...
  opeFilterDialog(): void {
    ...
    const dialogRef = this.dialogFilter.open(FilterNameSprComponent, {
      width: '350px',
      data: {name: this.filterTumanlar.name, SPR: 'Туман номи'}
    });
    dialogRef.afterClosed().subscribe(result => {
      ...
      if (result.name && result.ok) {
        this.filterTumanlar.name = result.name;
        this.getByFilter();
      } else {
        if (!result.close) {
          this.filterTumanlar.name = null;
          this.getAll();
          this.EnableButton();
        }
      }
  ...
  creatAll(ObjectForm) {
    if (this.positionId) {
      ...
      this.apiService.updated<Tumanlar>(this.className, ObjectForm, ObjectForm._id)
        .subscribe(
      ...
      this.apiService.creted<Tumanlar>(this.className, ObjectForm)
        .subscribe(
  ...
  getAll() {
    this.reloading = true;
    const params = Object.assign({}, this.filterTumanlar, {
      page: this.page,
      pageSize: this.pageSize
    });
    this.apiService.getAllByPagePost<PageResponse<Tumanlar>>(params, this.className)
      .subscribe(result => {
  ...
  getByFilter() {
    ...
    this.apiService.getFilter<PageResponse<Tumanlar>>(params, this.className)
      .subscribe(result => {
        this.list = result.content;
        this.totalDocs = result.totalDocs;
        this.hasPrevPage = result.hasPrevPage;
