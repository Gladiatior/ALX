...
import {UlchovBirligi} from '../../classes/interfaceSpr';
...
@Component({
  selector: 'app-ulchov-birligi',
  templateUrl: './ulchov-birligi.component.html',
  styleUrls: ['../../../assets/styles/spravochniks.css']
})
export class UlchovBirligiComponent implements OnInit {
  displayedColumns: string[] = ['id', 'nomi'];
  filterUlchovBirligi: FilterName = {};

  Buttonlar: boolean;
  form: FormGroup;
  tartibRaqamLength: number;
  isVisbleForm = false;
  selectedItem: UlchovBirligi;
  reloading = false;
  positionId: string;
  className = 'ulchovBirligi';
  list: UlchovBirligi;
  page = 1;
  pageSize = 10;
  totalPages: number;
  ...
  opeFilterDialog(): void {
    const dialogRef = this.dialogFilter.open(FilterNameSprComponent, {
      width: '350px',
      data: {name: this.filterUlchovBirligi.name, SPR: 'Омбор номи'}
    });
    dialogRef.afterClosed().subscribe(result => {
      ...
      if (result.name && result.ok) {
        this.filterUlchovBirligi.name = result.name;
        this.getByFilter();
      } else {
        if (!result.close) {
          this.filterUlchovBirligi.name = null;
          this.getAll();
          this.EnableButton();
        }
      }
  ...
  onSubmit() {
    const newUlchovBirligi: UlchovBirligi = {
      name: this.form.value.name,
      tartibRaqam: this.form.value.tartibRaqam,
    };
    if (newUlchovBirligi) {
      this.creatAll(newUlchovBirligi);
  ...
  creatAll(ObjectForm) {
    if (this.positionId) {
      ...
      this.apiService.updated<UlchovBirligi>(this.className, ObjectForm, ObjectForm._id)
        .subscribe(
      ...
      this.apiService.creted<UlchovBirligi>(this.className, ObjectForm)
        .subscribe(
  ...
  getAll() {
    this.reloading = true;
    const params = Object.assign({}, this.filterUlchovBirligi, {
      page: this.page,
      pageSize: this.pageSize
    });
    this.apiService.getAllByPagePost<PageResponse<UlchovBirligi>>(params, this.className)
      .subscribe(result => {
  ...
  getByFilter() {
    ...
    this.apiService.getFilter<PageResponse<UlchovBirligi>>(params, this.className)
      .subscribe(result => {
        this.list = result.content;
        this.totalDocs = result.totalDocs;
        this.hasPrevPage = result.hasPrevPage;
