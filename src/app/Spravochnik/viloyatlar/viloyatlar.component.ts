...
import {Viloyatlar} from '../../classes/interfaceSpr';
...
@Component({
  selector: 'app-viloyatlar',
  templateUrl: './viloyatlar.component.html',
  styleUrls: ['../../../assets/styles/spravochniks.css']
})
export class ViloyatlarComponent implements OnInit {
  displayedColumns: string[] = ['id', 'nomi'];
  filterViloyatlar: FilterName = {};

  Buttonlar: boolean;
  form: FormGroup;
  tartibRaqamLength: number;
  isVisbleForm = false;
  selectedItem: Viloyatlar;
  reloading = false;
  positionId: string;
  className = 'viloyatlar';
  list: Viloyatlar;
  page = 1;
  pageSize = 10;
  totalPages: number;
  ...
  opeFilterDialog(): void {
    const dialogRef = this.dialogFilter.open(FilterNameSprComponent, {
      width: '350px',
      data: {name: this.filterViloyatlar.name, SPR: 'Вилоят номи'}
    });
    dialogRef.afterClosed().subscribe(result => {
      ...
      if (result.name && result.ok) {
        this.filterViloyatlar.name = result.name;
        this.getByFilter();
      } else {
        if (!result.close) {
          this.filterViloyatlar.name = null;
          this.getAll();
          this.EnableButton();
        }
      }
  ...
  onSubmit() {
    const newViloyatlar: Viloyatlar = {
      name: this.form.value.name,
      tartibRaqam: this.form.value.tartibRaqam,
    };
    if (newViloyatlar) {
      this.creatAll(newViloyatlar);
  ...
  creatAll(ObjectForm) {
    if (this.positionId) {
      ...
      this.apiService.updated<Viloyatlar>(this.className, ObjectForm, ObjectForm._id)
        .subscribe(
      ...
      this.apiService.creted<Viloyatlar>(this.className, ObjectForm)
        .subscribe(
  ...
  getAll() {
    this.reloading = true;
    const params = Object.assign({}, this.filterViloyatlar, {
      page: this.page,
      pageSize: this.pageSize
    });
    this.apiService.getAllByPagePost<PageResponse<Viloyatlar>>(params, this.className)
      .subscribe(result => {
  ...
  getByFilter() {
    ...
    this.apiService.getFilter<PageResponse<Viloyatlar>>(params, this.className)
      .subscribe(result => {
        this.list = result.content;
        this.totalDocs = result.totalDocs;
        this.hasPrevPage = result.hasPrevPage;
