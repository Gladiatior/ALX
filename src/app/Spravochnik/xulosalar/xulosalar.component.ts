...
import {Xulosalar} from '../../classes/interfaceSpr';
...
@Component({
  selector: 'app-xulosalar',
  templateUrl: './xulosalar.component.html',
  styleUrls: ['../../../assets/styles/spravochniks.css']
})
export class XulosalarComponent implements OnInit {

  displayedColumns: string[] = ['id', 'nomi'];
  filterXulosalar: FilterName = {};

  Buttonlar: boolean;
  form: FormGroup;
  tartibRaqamLength: number;
  isVisbleForm = false;
  selectedItem: Xulosalar;
  reloading = false;
  positionId: string;
  className = 'xulosalar';
  list: Xulosalar;
  ...
  opeFilterDialog(): void {
    const dialogRef = this.dialogFilter.open(FilterNameSprComponent, {
      width: '350px',
      data: {name: this.filterXulosalar.name, SPR: 'Хулоса номи'}
    });
    dialogRef.afterClosed().subscribe(result => {
      ...
      if (result.name && result.ok) {
        this.filterXulosalar.name = result.name;
        this.getByFilter();
      } else {
        if (!result.close) {
          this.filterXulosalar.name = null;
          this.getAll();
        }
      }
    });
  ...
  onSubmit() {
    const newXulosalar: Xulosalar = {
      name: this.form.value.name,
      tartibRaqam: this.form.value.tartibRaqam,
    };
    if (newXulosalar) {
      this.creatAll(newXulosalar);
  ...
  creatAll(ObjectForm) {
    if (this.positionId) {
      ...
      this.apiService.updated<Xulosalar>(this.className, ObjectForm, ObjectForm._id)
        .subscribe(
      ...
      this.apiService.creted<Xulosalar>(this.className, ObjectForm)
        .subscribe(
  ...
  getAll() {
    this.reloading = true;
    const params = Object.assign({}, this.filterXulosalar, {
      page: this.page,
      pageSize: this.pageSize
    });
    this.apiService.getAllByPagePost<PageResponse<Xulosalar>>(params, this.className)
      .subscribe(result => {
  ...
  getByFilter() {
    ...
    this.apiService.getFilter<PageResponse<Xulosalar>>(params, this.className)
      .subscribe(result => {
        this.list = result.content;
        this.totalDocs = result.totalDocs;
        this.hasPrevPage = result.hasPrevPage;
