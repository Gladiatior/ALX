...
import {ZararkunandalarTuri} from '../../classes/interfaceSpr';
...
@Component({
  selector: 'app-zararkunandalar-turi',
  templateUrl: './zararkunandalar-turi.component.html',
  styleUrls: ['../../../assets/styles/spravochniks.css']
})
export class ZararkunandalarTuriComponent implements OnInit {
  displayedColumns: string[] = ['id', 'nomi'];
  filterZararkunandalarTuri: FilterName = {};

  Buttonlar: boolean;
  form: FormGroup;
  tartibRaqamLength: number;
  isVisbleForm = false;
  selectedItem: ZararkunandalarTuri;
  reloading = false;
  positionId: string;
  className = 'zararkunandalarTuri';
  list: ZararkunandalarTuri;
  page = 1;
  pageSize = 10;
  totalPages: number;
  totalDocs: number;
  ...
  opeFilterDialog(): void {
    const dialogRef = this.dialogFilter.open(FilterNameSprComponent, {
      width: '350px',
      data: {name: this.filterZararkunandalarTuri.name, SPR: 'Зараркунанда тури номи'}
    });
    dialogRef.afterClosed().subscribe(result => {
      ...
      if (result.name && result.ok) {
        this.filterZararkunandalarTuri.name = result.name;
        this.getByFilter();
      } else {
        if (!result.close) {
          this.filterZararkunandalarTuri.name = null;
          this.getAll();
          this.EnableButton();
        }
      }
  ...
  onSubmit() {
    const newZararkunandalarTuri: ZararkunandalarTuri = {
      name: this.form.value.name,
      tartibRaqam: this.form.value.tartibRaqam,
    };
    if (newZararkunandalarTuri) {
      this.creatAll(newZararkunandalarTuri);
  ...
  onResetForm() {
    this.filterZararkunandalarTuri = {};
    this.selectedItem = null;
    this.isVisbleForm = false;
    this.getAll();
    this.EnableButton();
  ...
  creatAll(ObjectForm) {
    if (this.positionId) {
      ...
      this.apiService.updated<ZararkunandalarTuri>(this.className, ObjectForm, ObjectForm._id)
        .subscribe(
      ...
      this.apiService.creted<ZararkunandalarTuri>(this.className, ObjectForm)
        .subscribe(
  ...
  getAll() {
    this.reloading = true;
    const params = Object.assign({}, this.filterZararkunandalarTuri, {
      page: this.page,
      pageSize: this.pageSize
    });
    this.apiService.getAllByPagePost<PageResponse<ZararkunandalarTuri>>(params, this.className)
      .subscribe(result => {
  ...
  getByFilter() {
    ...
    this.apiService.getFilter<PageResponse<ZararkunandalarTuri>>(params, this.className)
      .subscribe(result => {
        this.list = result.content;
        this.totalDocs = result.totalDocs;
        this.hasPrevPage = result.hasPrevPage;
